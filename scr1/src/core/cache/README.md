...
- I-cache: `Critical Word First` и `Early Restart` при заполнении строки;
- D-cache: последовательное заполнение всей строки словами из памяти;
...
I-cache принимает только чтения. При miss строка остаётся невалидной до полного
заполнения, однако слова запрашиваются по политике Critical Word First: первым
запрашивается слово, которое вызвало miss, после чего refill циклически обходит
остальные слова строки.

Early Restart выдаёт ответ процессору сразу после успешного получения
критического слова, не ожидая окончания refill. Критическое слово одновременно
записывается в data-массив и сохраняется как данные ответа. При этом
I-cache остаётся blocking: новый запрос ядра не принимается, пока фоновое
заполнение текущей строки не завершится или не будет прервано.

При запросе инвалидации после раннего ответа остаток refill прекращается на
границе транзакции памяти. Если ответ памяти уже ожидается, I-cache сначала
принимает его, затем переходит к инвалидации. Незавершённая строка не становится
валидной, а подтверждение инвалидации выдаётся после очистки valid-битов.
...
- четыре одиночных чтения на fill, без burst;
- Critical Word First и Early Restart реализованы только в I-cache;
- после раннего ответа I-cache не принимает следующий запрос до завершения оставшейся части refill;
...
### cache_1.4 - Critical Word First и Early Restart в I-cache

- Были добавлены `critical word first` и `early restart` оптимизации;

> Данные оптимизации хорошо изложены в книге Computer Architecture A Quantative Approach by John L. Hennesy David A. Patterson  
...
1. После версии 1.4 инвалидация перестала работать. На данный момент в процессе исправления.  